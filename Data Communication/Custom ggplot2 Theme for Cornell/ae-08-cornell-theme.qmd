...
-   [x] A white background with grid lines on the $y$-axis only.
-   [x] Ensure the grid lines on the $y$-axis are clearly visible.
-   [x] An appropriate serif font for the chart. Since the official font family is *Palantino* and proprietary to Adobe, we will use the free Google font [*Spectral*](https://fonts.google.com/specimen/Spectral) instead.
-   [x] The title and subtitle should be centered over the entire plot, not just the plot panel (i.e. the portion of the chart where the data is displayed).
...
cornell_degrees_plot +
  ...
  theme(
    ...
    panel.grid.minor.y = element_line(linewidth = 0.15, color ="grey"),
...
# First define our theme function
theme_cornell <- function(base_size = 12, base_family = "Spectral") {
  theme_minimal(
    base_size = base_size,
    ...
    theme(
      plot.title.position = "plot",
      plot.title = element_text(hjust = .5, size = base_size + 2),
      plot.subtitle = element_text(hjust = .5, size = base_size + 1),
      legend.position = "bottom",
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank(),
      panel.grid.major.y = element_line(linewidth = 0.3, color = "grey"),
...
# original plot
cornell_degrees_plot +
  scale_fill_manual(values = cornell_colors) +
  theme_cornell()

# scatter plot with iris data
ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  geom_point() +
  scale_color_manual(values = cornell_colors[1:3]) +
